...
void TrackerApp::finish() {
    log() << "finish()\n";
    // cleanup all the sockets
    this->socketMap_.deleteSockets();
}

void TrackerApp::socketEstablished(int connID, void* role) {
    if (role != NULL)
...
void TrackerApp::socketDataArrived(int connID, void *, cPacket *msg, bool) {
...
void TrackerApp::socketPeerClosed(int connId, void *) {
    log() << "socketPeerClosed(" << connId << ")\n";
}

void TrackerApp::socketClosed(int connId, void *) {
    log() << "socketClosed(" << connId << ")\n";
    setStatusString("closed");
}

void TrackerApp::socketFailure(int connId, void *, int code) {
    log() << "socketFailure(" << connId << "," << code << ")\n";
    setStatusString("broken");
}

// close the peer side
void TrackerApp::close() {
    log() << "close()\n";
...
// send a response
void TrackerApp::sendResponse(int connId, const char *id, unsigned long size) {
...
void TrackerApp::setStatusString(const char *s) {
    if (ev.isGUI ()) {
...
std::string TrackerApp::getPeersAsString(){
    std::stringstream ss;
